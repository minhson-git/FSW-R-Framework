...
def render_glyph(symbol: FSWRenderableSymbol, height: int = 160) -> Image.Image:
    """The symbol's real ISWA glyph as a cropped, upscaled RGBA image."""
    raw = visualize_sign(f"M500x500{_fsw_key(symbol)}480x480").convert("RGBA")
    bbox = raw.getbbox()
...
def render_glyph_to_file(symbol: FSWRenderableSymbol, output_path: str, title: str | None = None) -> None:
    ...
    pad, label_h = 12, 18
    canvas = Image.new("RGB", (glyph.width + 2 * pad, glyph.height + 2 * pad + label_h), (255, 255, 255))
    canvas.paste(glyph, (pad, pad + label_h), glyph)
    ImageDraw.Draw(canvas).text((pad, 2), title or symbol.symbol_id, fill=(0, 0, 0))
    canvas.save(output_path)


def render_glyphs_grid(symbols: Sequence[tuple[FSWRenderableSymbol, str]], output_path: str, height: int = 150) -> None:
    ...
    for i, (glyph, title) in enumerate(glyphs):
        x = i * cell_w + (cell_w - glyph.width) // 2
        canvas.paste(glyph, (x, pad + label_h), glyph)
        draw.text((i * cell_w + pad, 2), title, fill=(0, 0, 0))
    canvas.save(output_path)
