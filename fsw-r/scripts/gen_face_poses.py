...
def build() -> dict[str, object]:
    out: dict[str, object] = {
        "_meta": {
            ...
            "fill_nuance": "STRUCTURE RESOLVED, VALUES NOT YET APPLIED (see scripts/"
                           "check_face_fill_semantics.py, which reproduces all of this). fill is NOT "
                           "decoration for Category 4: 0 of 55 authored bases have the same glyph outline "
                           "across their fills. Two axes are measurable -- (a) FACIAL CIRCLE, 55/55 bases "
                           "have fills differing >=3x in glyph area, i.e. some draw the head outline "
                           "around the mark and some draw the mark alone; (b) SIDE, 14/55 bases have a "
                           "twin fill pair (same contour count, same bounding box, different outline) "
                           "sitting on opposite sides of the facial circle's centre. Textbook naming "
                           "agrees: 'Lessons in SignWriting' pp.158-159 lists 'Eyebrows Up / Right "
                           "Eyebrow Up / Left Eyebrow Up', and right/left are NOT separate base symbols "
                           "(0x30a/0x30b/0x30c are Up/Neutral/Down), so they can only be fills. "
                           "CONSEQUENCE: on those 14 bases a one-sided expression is currently stored as "
                           "a symmetric one, which ARKit-52 could express correctly "
                           "(browOuterUpLeft/Right, eyeBlinkLeft/Right). NOT applied yet because mapping "
                           "a viewer-side onto ARKit's character-anatomical side needs the face viewpoint "
                           "convention (expressive vs receptive) settled, and this project routes "
                           "unresolved left/right to AnnotationSymbol rather than guess.",
