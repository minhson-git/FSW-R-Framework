...
is exactly one of two kinds:
  ...
  * ``AnnotationSymbol``: a labelled marker with no modelled pose -- the
    ...
    *movements*, head symbols, and angled "dreamy" brows (see
    ``annotation_symbol.py``).
New modelling flips a base from AnnotationSymbol to FaceSymbol (as eyegaze
did) and the counts below move together -- so this test also pins the split.
"""
...
def test_every_base_builds_as_face_or_annotation(base_hex: int) -> None:
    symbol = symbol_from_fsw(_first_valid_key(base_hex))
    assert isinstance(symbol, (FaceSymbol, AnnotationSymbol))


def test_face_vs_annotation_split_is_pinned() -> None:
    face = 0
    annotation = 0
    ...
    assert face + annotation == 110  # every base decided, none left out
